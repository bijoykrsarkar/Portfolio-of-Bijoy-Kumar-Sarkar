document.addEventListener("DOMContentLoaded", () => {
    ...
    if (canvas) {
        ...
        function resize() {
            ...
            // Bug fix: dynamically evaluate on init/resize instead of globally
        ...
        function initStars() {
            ...
            const starCount = isMobileViewport ? 80 : 150;
            stars = [];
    ...
    if (cmdkTrigger && cmdkOverlay && cmdkInput && cmdkList) {
        const commands = [
            ...
            { label: 'Download Resume (PDF)',     icon: 'fa-solid fa-file-pdf',         action: () => {
                const a = document.createElement('a');
                a.href = 'Bijoy_Kumar_Sarkar_CV.pdf';
                a.download = 'Bijoy_Kumar_Sarkar_CV.pdf';
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
            }},
    ...
    function playClickSound() {
        try {
            ...
            // Limit sound triggers to prevent overlapping audio clipping
            const nowTime = performance.now();
            if (nowTime - lastClickTime < 100) return; 
